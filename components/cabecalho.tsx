...
import { BookOpen, Download, Newspaper, Menu, Bot } from "lucide-react";
...
export default function Cabecalho() {
  ...
  return (
    <>
      ...
      <header className="hidden md:flex bg-amber-100 px-6 py-3 items-center justify-between shadow-md sticky top-0 z-20">
        ...
        <nav className="flex items-center gap-6 relative">
          ...
          <Link href={todayUrl} className="relative px-1">
            ...
            <span className={`flex items-center gap-2 font-semibold ${isActive("/liturgia") ? "text-amber-700" : "text-amber-900"}`}>
              <BookOpen /> Liturgia
            </span>
          </Link>
          ...
          <Link href="/blog" className="relative px-1">
            ...
            <span className={`flex items-center gap-2 font-semibold ${isActive("/blog") ? "text-amber-700" : "text-amber-900"}`}>
              <Newspaper /> Blog
            </span>
          </Link>
      ...
      {/* MOBILE BOTTOM BAR */}
      <nav className="fixed md:hidden bottom-0 left-0 right-0 z-30 bg-white border-t shadow-2xl grid grid-cols-4 h-20 px-1">
        ...
        {/* Liturgia */}
        <Link href={todayUrl} className={`${baseBtn} ${isActive("/liturgia") ? activeBtn : normalBtn}`}>
          {isActive("/liturgia") && (
            <motion.span
              layoutId="mobile-active"
              className="absolute top-0 left-3 right-3 h-[3px] bg-amber-700 rounded-full"
        ...
        {/* Instalar */}
        {(installable || isIOS) ? (
          <button onClick={handleInstallClick} className={`${baseBtn} ${normalBtn}`}>
            <Download size={24} />
            <span className="text-[11px] font-medium">Instalar</span>
          </button>
        ) : (
          <button className={`${baseBtn} opacity-40`}>
            <Menu size={24} />
